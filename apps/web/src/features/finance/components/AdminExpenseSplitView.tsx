...
import { RoutineDuesModal } from './RoutineDuesModal';
...
export const AdminExpenseSplitView: React.FC<AdminExpenseSplitViewProps> = ({
  ...
  // Group units by block / prefix
  const blockSummary = useMemo(() => {
    const counts: Record<string, number> = {};
    unitList.forEach((u) => {
      const trimmed = (u.unit || '').trim();
      const match = trimmed.match(/^([A-Za-z0-9ÇĞİÖŞÜçğıöşü\s]+Blok|\bBlok\s+[A-Za-z0-9]+|[A-Za-z]\s*Blok)/i);
      let key = match ? match[0].trim() : trimmed.includes(' ') ? trimmed.split(' ')[0] : trimmed;
      if (!key) key = 'Genel';
      counts[key] = (counts[key] || 0) + 1;
    });

    return Object.entries(counts).sort((a, b) => a[0].localeCompare(b[0], 'tr'));
  }, [unitList]);

  // Filtered units for the modal
  const filteredModalUnits = useMemo(() => {
    if (!unitsSearchQuery.trim()) return unitList;
  ...
  return (
    <div className="space-y-6 max-w-full animate-fade-in pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-3 border-b border-slate-200">
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          <button
            ...
            className="h-10 inline-flex items-center gap-2 px-4 text-xs font-bold text-amber-900 bg-amber-50 hover:bg-amber-100 border border-amber-200/90 rounded-lg transition-colors cursor-pointer shadow-2xs"
          >
            <Zap size={14} className="text-amber-600 fill-amber-500/20" />
            <span>⚡ Rutin Aidat Oluştur</span>
          </button>
        </div>
      </div>
        ...
        <div className="bg-white border border-slate-300 border-t-3 border-t-teal-700 rounded-xl p-5 shadow-xs flex flex-col justify-between">
          <div>
            ...
            <div className="mt-4">
              ...
              {/* Blok Dağılımı Rozetleri */}
              {blockSummary.length > 0 ? (
                <div className="mt-2.5 space-y-2">
                  <div className="flex flex-wrap items-center gap-1.5">
                    {blockSummary.slice(0, 5).map(([block, count]) => (
                      <span
                        key={block}
                        className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-100 border border-slate-200/80 text-[11px] font-semibold text-slate-700 shadow-2xs"
                      >
                    ...
                    {blockSummary.length > 5 && (
                      <span className="text-[11px] font-semibold text-slate-500">
                        +{blockSummary.length - 5} blok daha
                      </span>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    className="text-[11px] font-semibold text-teal-700 hover:text-teal-900 hover:underline flex items-center gap-1 cursor-pointer transition-colors"
                  >
                    <span>Tüm daire listesini incele ({totalUnits})</span>
                    <ChevronRight size={12} />
                  </button>
                </div>
              ) : (
                <div className="text-xs text-slate-500 mt-1.5">
                  Henüz kayıtlı daire yok
                </div>
              )}
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button
                  type="button"
                  onClick={() => setIsRoutineModalOpen(true)}
                  className="text-xs font-bold text-amber-900 bg-amber-50 hover:bg-amber-100 border border-amber-200/90 px-2.5 py-1 rounded-lg cursor-pointer transition-colors flex items-center gap-1 shadow-2xs"
                  title="Bu ayın rutin aidatını oluştur"
                >
                  <Zap size={12} className="text-amber-600 fill-amber-500/20" />
                  <span>Aidat Üret</span>
                </button>
              </div>
            </div>
            <div className="text-sm text-slate-500 mt-1 font-medium">
      ...
      {/* 5. RUTİN AİDAT TAHAKKUK SİHİRBAZI MODALI */}
      <RoutineDuesModal
        ...
        periods={periods}
        onGenerate={autoGenerateMonthlyDues}
        onSuccessCreated={() => {
          triggerFinanceUpdate();
        }}
        onNavigateToDebts={() => {
          const tenantSlug = activeGroup?.slug || user?.group?.slug || 'demo';
          navigate(`/${tenantSlug}/admin/debts`);
        }}
        onOpenSettings={() => {
      ...
      {/* 6. KAYITLI DAİRELER LİSTESİ DETAY MODALI */}
      {isUnitsModalOpen &&
        createPortal(
          <div
            onClick={(e) => {
              if (e.target === e.currentTarget) setIsUnitsModalOpen(false);
            }}
          ...
          >
            <div className="bg-white rounded-2xl max-w-2xl w-full shadow-2xl border border-slate-200 animate-scale-up overflow-hidden my-auto flex flex-col max-h-[85vh]">
              {/* Modal Başlığı */}
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-900 text-sm">
                      Kayıtlı Bağımsız Bölümler ({totalUnits} Daire)
                    </h3>
                    <p className="text-[11px] text-slate-500">
                      Gider ve aidat paylaştırmasına dahil olan bağımsız bölümler
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="p-4 border-b border-slate-100 bg-white">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Daire veya sakin adı ile ara..."
                    className="w-full pl-9 pr-8 py-2 border border-slate-200 rounded-xl text-xs font-medium text-slate-900 focus:border-teal-600 focus:ring-1 focus:ring-teal-600 outline-none"
                  />
                  {unitsSearchQuery && (
                    <button
                      type="button"
                      onClick={() => setUnitsSearchQuery('')}
                ...
                {filteredModalUnits.length > 0 ? (
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {filteredModalUnits.map((u) => (
                      <div
                        key={u.id}
                        className="p-2.5 rounded-lg border border-slate-200/80 bg-slate-50/50 hover:bg-teal-50/30 hover:border-teal-200 transition-all flex items-center justify-between gap-2"
                      >
                        <div className="min-w-0">
                          ...
                          <div className="text-[10px] text-slate-500 truncate mt-0.5">
                            {u.residentName || 'Boş Daire'}
                          </div>
                        </div>
                        <span
                          className={`text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0 ${
                            u.residentType === 'owner'
                              ? 'bg-amber-50 text-amber-800 border border-amber-200'
                              : 'bg-indigo-50 text-indigo-800 border border-indigo-200'
                          }`}
                        >
                          {u.residentType === 'owner' ? 'Ev Sahibi' : 'Kiracı'}
                        </span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="text-center py-8 text-xs text-slate-400">
                    Aramanıza uygun daire bulunamadı.
                  </div>
                )}
              </div>
              ...
              <div className="px-6 py-3 border-t border-slate-100 bg-slate-50 flex items-center justify-between text-xs">
                <span className="text-slate-500 text-[11px]">
                  Toplam <strong>{filteredModalUnits.length}</strong> daire listeleniyor
                </span>
                <button
                  type="button"
                  onClick={() => setIsUnitsModalOpen(false)}
                  className="px-4 py-1.5 bg-white border border-slate-200 text-slate-700 font-semibold rounded-lg hover:bg-slate-100 cursor-pointer transition-colors"
                >
                  Kapat
                </button>
              </div>
